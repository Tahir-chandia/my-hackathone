...
const Newarrival = () => {
  ...
  useEffect(()=>{
    async function fetchProduct() {
     const fetchData:Products[] = await client.fetch(four)
...
<section className="max-w-screen-2xl  mx-auto   ">
    ...
    <div className=' px-[80px] mt-[200px]'>
      <div >
        <h2 className='font-IntegralBold text-[48px] text-black text-center '>New Arrivals</h2>
      </div>
      <div className=" flex space-x-4 mt-[50px]">
    
        {product.map((pro)=>(
            <div className="space-y-2" key={pro._id}> 
<Link href={`/products/${pro._id}`}>
           ...
           <div>
                     ...
                     <div className="flex mb-1 font-Satoshi">
                                                 {star.map((icon, index) => (
                                                     <span className="text-yellow-400" key={index}>{icon}</span>
...
<div className="w-full mt-[50px] border-b-[1px] border-b-black  border-opacity-[10%]">
